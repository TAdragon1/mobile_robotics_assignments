...
void DesStatePublisher::pub_next_state() {
    ...
    if (e_stop_trigger_) {
        ...
        trajBuilder_.build_braking_traj(current_pose_, des_state_vec_, des_state_);
    ...
    switch (motion_mode_) {
        ...
        case HALTING: //e-stop service callback sets this mode
            ...
            ROS_WARN("Halting!");
            ROS_INFO("Num of trajectories = %d", npts_traj_);
            ROS_INFO("Current index = %d",traj_pt_i_);
            ...
            if (traj_pt_i_ >= npts_traj_ - 1) { //here if completed all pts of braking traj

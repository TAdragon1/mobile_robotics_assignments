...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    //Takes 2.0 seconds to decel to full stop from max for velocity 
    nav_msgs::Odometry des_state;
    des_state.header = start_pose.header; //really, want to copy the frame_id
    des_state.pose.pose = start_pose.pose; //start from here
    ...
    double decel_max = -1.0*accel_max_;
    double alpha_neg = -1.0*alpha_max_;

    double x_curr = start_pose.pose.position.x;
    double y_curr = start_pose.pose.position.y;
    double psi_start = convertPlanarQuat2Psi(start_pose.pose.orientation);
    double psi_des = psi_start;

    //assume we start at max possible speeds
    double vel_x_cur = speed_max_;
    double vel_y_cur = speed_max_;
    double ang_cur = omega_max_;
    ...
    double t = 0.0;
    while(vel_x_cur > 0 && vel_y_cur > 0 && ang_cur > 0){
        t += dt_;

        if(t >= 5.0){
            ROS_WARN("Took too long, pushing back halted twist!");
            des_state.twist.twist = halt_twist_;
            vec_of_states.push_back(des_state);
            break;
        }

        //apply max decel to current velocity over dt and solve for distance traveled
        if(vel_x_cur > 0){
            dist_x = vel_x_cur*dt_ + 0.5*(decel_max*dt_*dt_);
            x_curr += dist_x;
            des_state.pose.pose.position.x = x_curr;
        }
        if(vel_y_cur > 0){
            dist_y = vel_y_cur*dt_ + 0.5*(decel_max*dt_*dt_);
            y_curr += dist_y;
            des_state.pose.pose.position.y = y_curr;
        }
        if(ang_cur > 0){
            dist_ang = ang_cur*dt_ + 0.5*(alpha_neg*dt_*dt_);
        }

        //use distance traveled to calculate the next current speeds
        vel_x_cur = sqrt(vel_x_cur*vel_x_cur + 2.0*decel_max*dist_x);
        vel_y_cur = sqrt(vel_y_cur*vel_y_cur + 2.0*decel_max*dist_y);
        ang_cur = sqrt(ang_cur*ang_cur + 2.0*alpha_neg*dist_ang);
        ...
        des_state.twist.twist.linear.x = vel_x_cur;
        des_state.twist.twist.linear.y = vel_y_cur;
        des_state.twist.twist.angular.z = ang_cur;

        vec_of_states.push_back(des_state);

        ROS_INFO("So far, Time = %f \nVelocity x = %f, Velocity y = %f, Angular Velocity = %f", t, vel_x_cur, vel_y_cur, ang_cur);
    }
        
    ROS_INFO("ESTOP HAS FINISHED");
}
